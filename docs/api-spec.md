...
| 状态码 | 场景 | 示例 |
|--------|------|------|
| 200 | 查询/更新成功 | `GET /users/me` 返回当前用户信息 |
| 201 | 创建成功 | `POST /rules` 创建规则后返回新规则 |
| 400 | 请求参数校验失败 | 缺少必填字段、格式错误 |
| 401 | Token 无效或过期 | 未携带 JWT、JWT 签名错误、已过期 |
| 403 | 权限不足 | OPERATOR 角色访问 admin 专属端点 |
| 404 | 资源不存在 | `GET /rules/999` 规则 ID 不存在 |
| 409 | 资源冲突 | 重复注册同名用户、删除被事件引用的规则 |
| 422 | 业务规则校验失败 | 状态机非法流转、数值超出业务允许范围 |
| 429 | 请求过于频繁 | 登录失败超过 5 次，账号被锁定 15 分钟 |
| 500 | 未预期的系统错误 | 已知业务边界不得使用 500，必须映射为 409/422/400 |
