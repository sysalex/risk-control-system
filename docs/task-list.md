...
- [x] 8.1 用户管理页面
- [x] 8.2 规则管理页面（CRUD、启停）
  - 完成时间：2026-04-28
  - 测试覆盖率：新增 UserManagementView、RuleManagementView 和管理员路由测试；前端 `pnpm vitest run` 通过
  - 审查状态：本地代码/安全自检通过
...
- [x] 9.1 风险事件列表/详情/处理页面
- [x] 9.2 风险评分/决策页面
- [x] 9.3 审计日志查看页面
  - 完成时间：2026-04-28
  - 测试覆盖率：新增 EventAnalysisView、ScoreDecisionView、AuditLogView 和分析师路由测试；前端 `pnpm vitest run` 通过
  - 审查状态：本地代码/安全自检通过
...
- [x] N.2 Harness Engineering 规范可信度优化
  ...
  - 审查状态：文档差异自检通过
- [x] N.3 上下文防规避规则补强
  - 范围：补充上下文重载、会话恢复、禁止选择性忽略长期状态的规范和检查项；登记质量门禁脚本误报技术债务
  - 类型：文档任务，不涉及代码、API 契约、数据库 schema、认证授权
  - 完成时间：2026-04-30
  - 审查状态：文档差异自检通过
