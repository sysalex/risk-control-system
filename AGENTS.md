...
长期状态以仓库文件为准，不能只依赖当前 prompt。Agent 必须主动加载与任务相关的规范、任务、规格、计划和交接记录，避免因上下文压缩、会话恢复或换 Agent 导致执行级别降低。

### 必须重载的上下文

- 每次开始非只读任务前，必须查阅 `docs/task-list.md`、本文件、`CLAUDE.md`、`docs/definition-of-done.md` 和相关 `docs/specs/` / `docs/plans/`。
- 会话恢复、上下文压缩、换 Agent、接手未完成任务或用户说“继续”时，必须先查阅 `session-handoff.md`、`docs/task-list.md`、最近相关规格/计划，再继续执行。
- 发现当前 prompt 与仓库文件中的长期状态不一致时，以优先级规则裁决；若涉及 API 契约、数据库 schema、认证授权、安全边界或生产配置，先停止并确认。

### 禁止的上下文规避

- 不得选择性忽略 `AGENTS.md`、`CLAUDE.md`、`docs/task-list.md`、SDD 产物或 `session-handoff.md` 中已有约束。
- 不得以“当前 prompt 没写”为理由，绕过文件中已有长期状态、任务依赖、风险边界或验收标准。
- 不得通过少读上下文、只读局部文件或刻意不查交接记录来降低 SDD / TDD / DoD / Code Review / Security Review 的执行级别。
- 不得在上下文不完整时标记任务完成；必须先补齐必要上下文或明确向用户报告缺口。
...
- ❌ 选择性忽略长期上下文，或通过少读规范/任务/交接记录来降低执行级别
