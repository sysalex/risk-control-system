...
- `application.dto` 下的 `*Request` / `*Response` record 属于 DTO，但对外承载接口契约，类级 Javadoc 必须包含职责描述、`@author harness-agent`、`@since YYYY-MM-DD`
- DTO 字段或 record 组件必须逐项添加 `/** */` 块级 Javadoc，格式与实体字段注释保持一致；复杂字段的示例和值域同步写入 `docs/api-spec.md`
- DTO 优先使用 `record` 或不可变类；record 已由 Java 生成访问器、构造器、`equals`、`hashCode` 和 `toString`，不使用 Lombok `@Data`。只有确有框架绑定或可变 JavaBean 需求时，才改为 class + `@Data`
